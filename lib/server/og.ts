...
export async function ogImage(card: SocialCard, env: Env): Promise<Response> {
  ...
  const titleSize = episode ? (card.title.length > 145 ? 48 : card.title.length > 110 ? 56 : card.title.length > 80 ? 64 : 76) : (card.title.length > 65 ? 68 : 88);
  ...
  const tree = box({ width: 1200, height: 630, position: 'relative', overflow: 'hidden', background: '#29324a', color: '#fff', fontFamily: 'Inter' },
    ...
    episode ? picture(artwork[0], 400.5, 534, { position: 'absolute', left: 48, top: 48, borderRadius: 14, objectFit: 'contain', boxShadow: '0 14px 48px rgba(0,0,0,0.22)' }) : null,
    ...
    box({ position: 'absolute', left: episode ? 496.5 : 64, top: 48, width: textWidth, height: 534, flexDirection: 'column' },
      picture(dataUrl(logo, 'image/svg+xml'), 60, 60),
      box({ flex: 1, flexDirection: 'column', justifyContent: 'center', paddingTop: 22, paddingBottom: 20 },
        box({ fontFamily: 'Reckless', fontSize: titleSize, lineHeight: 1.04, letterSpacing: '-0.8px', maxHeight: 366, overflow: 'hidden' }, title)),
      box({ borderTop: '1px solid rgba(255,255,255,0.23)', paddingTop: 24, alignItems: 'center', gap: 14, fontSize: 22 },
        ...
        episode ? box({ marginLeft: 'auto', fontSize: 20, opacity: 0.8 }, 'Stories worth listening.') : null)));
