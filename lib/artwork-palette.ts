...
export function readableArtworkColor({ r, g, b }: ArtworkColor): string {
  ...
  let channels = [r, g, b].map(value => Math.round(Math.max(0, Math.min(255, midpoint + (value - midpoint) * 1.18)) * 0.92));
  ...
  while (luminance(channels) > 0.14) channels = channels.map(value => Math.floor(value * 0.95));
...
export async function extractArtworkPalette(image: HTMLImageElement): Promise<string[] | null> {
  ...
  const colors = getPaletteSync(canvas, { colorCount: 5, quality: 5 });
