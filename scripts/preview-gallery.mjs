...
const types = { '.css': 'text/css', '.html': 'text/html', '.js': 'text/javascript', '.json': 'application/json', '.webp': 'image/webp', '.svg': 'image/svg+xml', '.otf': 'font/otf', '.png': 'image/png' };
createServer(async (req, res) => {
    const url = new URL(req.url, 'http://127.0.0.1:4173');
    try {
        if (url.pathname === '/api/categories') {
            res.setHeader('Content-Type', 'application/json');
            res.end(JSON.stringify({ success: true, data: [...new Set(catalog.data.map(item => item.category).filter(Boolean))].sort() }));
            return;
        }
        ...
        const path = url.pathname === '/home-v2' ? resolve('legacy/home-v2.html') : extname(file) ? file : resolve('legacy/index.html');
        res.setHeader('Content-Type', types[extname(path)] || 'application/octet-stream');
        res.setHeader('Cache-Control', 'no-store');
        res.end(await readFile(path));
    } catch { res.writeHead(404).end('Not found'); }
}).listen(port, '127.0.0.1', () => console.log(`Gallery QA: http://127.0.0.1:${port} (add ?count=1000 or ?count=3)`));
