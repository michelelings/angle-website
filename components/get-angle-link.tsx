...
export function GetAngleLink({ location, episodeId }: { location: 'header' | 'modal'; episodeId?: string }) {
  return <a href={APP_URL} className={location === 'header' ? 'button' : 'audio-get-angle-btn'}
    ...
    }}>{location === 'modal' && <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 3v12m-4-4 4 4 4-4M5 16v5h14v-5" /></svg>}<span className="episode-action-label">Get Angle</span></a>;
}
